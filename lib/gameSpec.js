var canvas = document.getElementById('canvas'),
	...
	pitchURL = window.getComputedStyle(canvas, null).getPropertyValue('background-image').replace("url(", "").replace(")", ""),
	canvasHeight = parseInt(window.getComputedStyle(canvas, null).getPropertyValue('height')),
	...
	{
		pitchHeight = myImg.naturalHeight;
	}
...
function shootBall()
{
	...
	if(shooter == "goal")
	{
		if(playerId == 0)
		{
			$('#football').animate({top: '54px', left: '404px'}, 300, "swing").addClass('ballAni');
			setPlayer(1);
			score1++;
		} else if(playerId == 1)
		{
...
function checkAns()
{
	...
	bgPosY = parseInt(window.getComputedStyle(canvas, null).getPropertyValue('background-position').split(" ")[1]);
	if(userAnswer == ""){
...
function shoot()
{
	canvas.style.backgroundPosition = bgPosX + " " + bgPosY + "px";
	setQuestion();
	if(level == 4 || level == -4)
	{
...
function toCenter()
{
	...
	document.getElementById('shoot-wrapper').style.display = "none";
	level = 0;
	canvas.style.backgroundPosition = "1px " + centerPos + "px";
...
//binary[question] to decimal[answer] conversion
function set4BtoD()
{
...
//decimal[question] to binary[answer] conversion
function setDto4B()
{
	var myObj = new Object();
...
a2=Math.floor(Math.random() * 14) + 1;
myObj.answer=a1-a2;
myObj.title="Subtract  " + a2 + "  from  " + a1;
return myObj;
}
